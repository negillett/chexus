...
def test_put_item(dryrun, caplog):
    """Can put TableItems"""
    ...
    if dryrun:
        ...
        assert "Put complete" in caplog.text


def test_put_item_duplicate(caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        client.put_items(item, "test_table")

    # Should've checked table for existing record...
    mocked_table.query.assert_called()
    # ...and found one
    assert "Item already exists in table" in caplog.text
    # Should not have tried to put
    mocked_table.put_item.assert_not_called()


def test_put_item_invalid_item(caplog):
    """Doesn't attempt to put invalid items"""
    ...
    with caplog.at_level(logging.DEBUG):
        client.put_items(items=items, table_name="test_table")
    ...
    client._session.resource().Table().put_items.assert_not_called()


def test_put_item_without_table_key(caplog):
    ...
    assert "Item to put is missing required key, 'Nope'" in caplog.text
